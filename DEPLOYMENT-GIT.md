...
```bash
# Shared database (used by both apps)
...
**Important**: 
- `potato-users` is **shared** between both apps - only create it **once**
- Save all database IDs for later configuration in `wrangler.toml` files
...
**Important**: Both apps share the same `potato-users` database. The `database_id` for `DB_USERS` must be **identical** in both `wrangler.toml` files.
...
database_id = "your-users-db-id"  # ⚠️ Must match diet app's DB_USERS database_id
...
binding = "DB_USERS"
database_name = "potato-users"
database_id = "your-users-db-id"  # ⚠️ Must match charts app's DB_USERS database_id
...
| **Build output directory** | `apps/charts/.next` |
| **Root directory** | `/` (leave blank or set to root) |

**Important**: 
- **Root directory** must be set to `/` (repository root) for monorepo builds
- **Framework preset**: Select `Next.js` - Cloudflare Pages will automatically detect and configure Next.js
- The build command:
  1. Installs dependencies from repository root (`pnpm install`)
  2. Changes to the charts app directory (`cd apps/charts`)
  3. Runs Next.js build (`pnpm build`) which creates `.next` directory
- Using `pnpm` ensures proper workspace resolution in the monorepo
- **Note**: We use Cloudflare Pages native Next.js support - no adapter needed!

### Step 3: Configure Environment Variables

Before deploying, configure environment variables for **Production** and **Preview** (Dev) environments.

#### Production Environment Variables

For the `main` branch (Production), add:

| Variable | Value | Type | Environment |
|----------|-------|------|-------------|
| `NEXTAUTH_SECRET` | Generate with `openssl rand -base64 32` | Secret | **Production** |
| `NEXTAUTH_URL` | `https://potato-charts.pages.dev` | Public | **Production** |
| `NODE_VERSION` | `20` | Public | **Production** |

#### Preview/Dev Environment Variables

For other branches (Dev/Preview), add:

| Variable | Value | Type | Environment |
|----------|-------|------|-------------|
| `NEXTAUTH_SECRET` | Generate with `openssl rand -base64 32` (different from Production) | Secret | **Preview** |
| `NEXTAUTH_URL` | `https://[preview-url].potato-charts.pages.dev` | Public | **Preview** |
| `NODE_VERSION` | `20` | Public | **Preview** |

**Important**:
- ⚠️ **Use different `NEXTAUTH_SECRET` for Production and Preview** - they must be different!
- The `NEXTAUTH_URL` for Preview will be auto-generated by Cloudflare (format: `https://[hash].potato-charts.pages.dev`)
- You can update `NEXTAUTH_URL` after first deployment
- See [ENVIRONMENT-SETUP.md](./ENVIRONMENT-SETUP.md) for detailed multi-environment configuration guide
...
4. Add two bindings:
   ...
   | Variable name | D1 Database | Note |
   |---------------|-------------|------|
   | `DB_USERS` | potato-users | Shared database (used by both apps) |
   | `DB_CHARTS` | potato-charts | Charts-specific database |

**Important**: Both apps use the same `potato-users` database. Ensure the database ID matches in both `wrangler.toml` files.
...
**Important**: The `potato-users` database is **shared** between both apps. You only need to migrate it **once** (either during charts or diet setup).

```bash
# Generate migration SQL for shared users database
...
# Generate migration SQL for charts-specific database
cd ../database-charts
npx prisma migrate diff \
  --from-empty \
  --to-schema-datamodel prisma/schema.prisma \
...
# Note: Only migrate potato-users once (skip if already migrated during diet setup)
...
| **Production branch** | `main` |
| **Framework preset** | `Next.js` |
| **Build command** | `pnpm install && cd apps/diet && pnpm build` |
| **Build output directory** | `apps/diet/.next` |
| **Root directory** | `/` (leave blank or set to root) |
...
- The build command:
  1. Installs dependencies from repository root (`pnpm install`)
  2. Changes to the diet app directory (`cd apps/diet`)
  3. Runs Next.js build (`pnpm build`) which creates `.next` directory
- **Note**: We use Cloudflare Pages native Next.js support - no adapter needed!

### Step 3: Configure Environment Variables

Add environment variables for **Production** and **Preview** (Dev) environments.
...
|----------|-------|------|-------------|
| `NEXTAUTH_SECRET` | Generate with `openssl rand -base64 32` | Secret | **Production** |
| `NEXTAUTH_URL` | `https://potato-diet.pages.dev` | Public | **Production** |
| `OPENROUTER_API_KEY` | Your OpenRouter API key | Secret | **Production** |
| `NODE_VERSION` | `20` | Public | **Production** |

#### Preview/Dev Environment Variables

For other branches (Dev/Preview):

| Variable | Value | Type | Environment |
|----------|-------|------|-------------|
| `NEXTAUTH_SECRET` | Generate with `openssl rand -base64 32` (different from Production) | Secret | **Preview** |
| `NEXTAUTH_URL` | `https://[preview-url].potato-diet.pages.dev` | Public | **Preview** |
| `OPENROUTER_API_KEY` | Your OpenRouter API key (can be same or different) | Secret | **Preview** |
| `NODE_VERSION` | `20` | Public | **Preview** |
...
|---------------|-------------|------|
| `DB_USERS` | potato-users | Shared database (used by both apps) |
| `DB_DIET` | potato-diet | Diet-specific database |
...
**Important**: The `potato-users` database is **shared** between both apps. If you already migrated it during charts setup, skip the users migration here.

```bash
# Generate migration SQL for shared users database (if not already migrated)
cd packages/database-users
npx prisma migrate diff \
  --from-empty \
  --to-schema-datamodel prisma/schema.prisma \
...
# Generate migration SQL for diet-specific database
cd ../database-diet
npx prisma migrate diff \
  --from-empty \
  --to-schema-datamodel prisma/schema.prisma \
...
# Apply migrations to D1
# Only migrate potato-users if charts app hasn't been deployed yet
# wrangler d1 execute potato-users --remote --file=packages/database-users/migration-users.sql
...
```bash
pnpm install && cd apps/charts && if git diff --quiet HEAD~1 apps/charts packages/; then echo "No changes"; exit 0; fi && pnpm pages:build
```
...
```yaml
build:
  command: pnpm install && cd apps/charts && pnpm build
  output_directory: apps/charts/.next

env:
  NODE_VERSION: 20
```

**Note**: `.pages.yaml` is optional. The build settings in the Cloudflare dashboard take precedence.
...
**Solution**:
- Verify build command includes `pnpm install` at the start
- Ensure `cd apps/[app-name]` is included before running `pnpm pages:build`
- Check `NODE_VERSION` is set to `20`
- Verify `pnpm` is available (Cloudflare Pages should auto-detect from `packageManager` field)

---

**Issue**: Build fails with "workspace not found" or "package not found"

**Solution**:
- Build command must start from **repository root** with `pnpm install`
- Use: `pnpm install && cd apps/charts && pnpm pages:build`
- Not: `cd apps/charts && pnpm pages:build` (missing root install)
- Ensure Root directory is set to `/` in Cloudflare Pages settings
- Verify `pnpm-workspace.yaml` exists in repository root
